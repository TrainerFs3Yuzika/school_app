                        ...
                        <div class="table-responsive">
                            <table id="studentTable" class="table border-0 star-student table-hover table-center mb-0 table-striped">
                                <thead class="student-thread">
                                    <tr>
                                        <th>No</th>
                                        @if (auth()->user()->role_name === 'Super Admin' || auth()->user()->role_name === 'Teachers' || auth()->user()->role_name === 'Student' || auth()->user()->role_name === 'Kepala Sekolah')
                                            <th>Guru</th>
                                        @endif
                                        <th>Murid</th>
                                    ...
                                    @foreach ($scores as $index => $score)
                                        <tr>
                                            ...
                                            @if ($score->score >= 80 && $score->score <= 100)
                                                <td><span class="bg-success opacity-50 px-2 py-1 rounded-pill text-white fw-bold">{{ $score->score }}</span></td>
                                            @elseif($score->score >= 70 && $score->score <= 79)
                                                <td><span class="bg-warning opacity-50 px-2 py-1 rounded-pill text-white fw-bold">{{ $score->score }}</span></td>
                                            @elseif($score->score >= 1 && $score->score <= 69)
                                                <td><span class="bg-danger opacity-50 px-2 py-1 rounded-pill text-white fw-bold">{{ $score->score }}</span></td>
                                            @endif
                                            @if (auth()->user()->role_name === 'Super Admin' || Session::get('role_name') === 'Teachers')
                                            <td class="text-center align-middle">
                                                <div class="actions d-flex justify-content-center align-items-center">
                                                    ...
                                                    <a href="{{ route('scores.edit', $score->id) }}" class="btn btn-sm bg-danger-light me-2">
                                                    ...
                                                    <a href="{{ route('scores.export.pdf', ['id' => $score->id]) }}" class="btn btn-sm bg-danger-light me-2">
